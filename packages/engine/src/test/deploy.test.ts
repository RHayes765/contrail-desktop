 ...
 *
 * Ported from the Phase 0 MCP-transport suite to direct capability
 * invocation. The presenter seam is now structured (ApprovalRequestView), so
 * the tests read codes/warnings off the captured view instead of grepping
 * rendered HTML; the raw-HTTP tests still hit the real localhost page, which
 * the wrapped ApprovalPageServer renders from the same view.
 */
 ...
 * the human, reading the code off the view) while delegating to a real
 * ApprovalPageServer so the localhost page — and its self-invalidation
 * endpoints — still exist for the raw-HTTP tests.
 */
class RecordingPresenter implements ApprovalPresenter {
  readonly views: ApprovalRequestView[] = [];
  readonly urls: string[] = [];
  private readonly inner: ApprovalPageServer;
  constructor(openBrowser: (url: string) => Promise<void>) {
    this.inner = new ApprovalPageServer(openBrowser);
  }
  async present(
    ...
    const p = await this.inner.present(view, statusCheck, ttlMs);
    this.urls.push(p.url ?? '');
    return p;
  }
...
beforeEach(async () => {
  ...
  presenter = new RecordingPresenter(async (url) => {
    openedUrls.push(url);
  });
...
async function validate(extra: Record<string, unknown> = {}) {
  return invokeCapability(deps, 'validate_deploy', {
...
describe('validate_deploy', () => {
  ...
  it('warns when a package adds components that need permissions but grants none', async () => {
    ...
    // The warning reaches the approval view too (rendered amber on the page).
    expect(presenter.views.at(-1)!.warnings?.join(' ')).toContain('invisible or inaccessible');
  });
...
describe('execute_deploy code lifecycle', () => {
  it('executes with the human-read code exactly once', async () => {
    ...
    const again = await invokeCapability(deps, 'execute_deploy', {
      connection: 'deploy-org',
      confirmation_code: code,
    });
  ...
  it('locks the pending code after too many wrong guesses (brute-force guard)', async () => {
    ...
    for (let i = 0; i < 4; i++) {
      const r = await invokeCapability(deps, 'execute_deploy', {
        connection: 'deploy-org',
        confirmation_code: `WRONG-CD${i}`,
      });
      expect(r.isError).toBe(true);
      expect(textOf(r)).toMatch(/attempt\(s\) remain/);
    }
    const fifth = await invokeCapability(deps, 'execute_deploy', {
      connection: 'deploy-org',
      confirmation_code: 'WRONG-CDX',
    });
    expect(textOf(fifth)).toContain('locked');

    // Even the CORRECT code no longer works — the human must re-validate.
    const correct = await invokeCapability(deps, 'execute_deploy', {
      connection: 'deploy-org',
      confirmation_code: code,
    });
  ...
  it('a new validation supersedes the previous code', async () => {
    await validate();
    const firstCode = presenter.views[0]!.code;
    await validate();
    const exec = await invokeCapability(deps, 'execute_deploy', {
      connection: 'deploy-org',
      confirmation_code: firstCode,
    });
    expect(exec.isError).toBe(true);
    expect(deployCounter.realDeploys).toBe(0);
    // the fresh code still works
    const secondCode = presenter.views[1]!.code;
    const exec2 = await invokeCapability(deps, 'execute_deploy', {
      connection: 'deploy-org',
  ...
  it('re-executing after completion returns the stored result, not an error or a second deploy', async () => {
    await validate();
    const code = presenter.views[0]!.code;
    const first = await invokeCapability(deps, 'execute_deploy', {
      connection: 'deploy-org',
      confirmation_code: code,
    });
    ...
    const late = await invokeCapability(deps, 'execute_deploy', {
      connection: 'deploy-org',
      confirmation_code: code,
    });
  ...
  it('concurrent execute_deploy with one code dispatches exactly one deploy', async () => {
    ...
    const [a, b] = await Promise.all([
      invokeCapability(deps, 'execute_deploy', {
        connection: 'deploy-org',
        confirmation_code: code,
      }),
      invokeCapability(deps, 'execute_deploy', {
...
describe('approval URL is never handed to the agent', () => {
  it('withholds the approval URL when the browser fails to open', async () => {
    // A presenter whose browser-open always throws.
    const failing = new RecordingPresenter(async () => {
...
describe('flow deactivation (P0.6)', () => {
  ...
  it('deactivate_flow validates a standalone deactivation and routes through approval', async () => {
    const result = await invokeCapability(deps, 'deactivate_flow', {
      connection: 'deploy-org',
      flow: 'My_Flow',
    });
    const text = textOf(result);
...
describe('dml two-step', () => {
  it('proposes with before/after preview and executes with the page code', async () => {
    const propose = await invokeCapability(deps, 'dml_propose', {
      connection: 'deploy-org',
      operation: 'update',
      object: 'Account',
  ...
  it('deletes render in the destructive section of the approval page', async () => {
    ...
    // The delete rows land in the destructive section, not the plain changes list.
    expect(view.destructive.length).toBeGreaterThan(0);
    expect(view.destructive.map((d) => d.label).join(' ')).toContain(
      'DELETE Account 001000000000001AAA',
    );
