...
describe('soql_query', () => {
  ...
  it('refuses metadata-bearing sObjects without metadata_read (dual-grant boundary)', async () => {
    const result = await invokeCapability(deps, 'soql_query', {
      ...
      query: 'SELECT Id, Body FROM ApexClass',
    });
    expect(result.isError).toBe(true);
...
describe('get_record', () => {
  it('fetches one record and strips attributes', async () => {
    const result = await invokeCapability(deps, 'get_record', {
      connection: 'data-org',
      object: 'Account',
      id: '001000000000001AAA',
    });
...
describe('get_debug_logs', () => {
  it('lists recent logs', async () => {
    const result = await invokeCapability(deps, 'get_debug_logs', { connection: 'data-org' });
  ...
  it('fetches a log body by id', async () => {
    const result = await invokeCapability(deps, 'get_debug_logs', {
      connection: 'data-org',
      log_id: '07L000000000001AAA',
    });
...
describe('run_apex_tests', () => {
  it('submits and returns a pollable run id with the rollback note', async () => {
    const result = await invokeCapability(deps, 'run_apex_tests', { connection: 'data-org', class_names: ['InvoiceServiceTest'] });
  ...
  it('demands exactly one of class_names, tests, test_run_id', async () => {
    const result = await invokeCapability(deps, 'run_apex_tests', {
        connection: 'data-org',
        class_names: ['A'],
  ...
  it('reports in-progress while the queue is still running', async () => {
    apexTestsDone = false;
    const result = await invokeCapability(deps, 'run_apex_tests', { connection: 'data-org', test_run_id: '707000000000001AAA' });
  ...
  it('rejects malformed class and method names before any org call', async () => {
    const bad = await invokeCapability(deps, 'run_apex_tests', { connection: 'data-org', class_names: ["Bad'Name"] });
    expect(bad.isError).toBe(true);
    const badMethod = await invokeCapability(deps, 'run_apex_tests', {
        connection: 'data-org',
  ...
  it('an aborted run is NEVER reported as a pass', async () => {
    ...
    const result = invokeCapability(deps, 'run_apex_tests', {
      connection: 'data-org',
      test_run_id: '707000000000001AAA',
    });
  ...
  it('is gated on diagnostics_read', async () => {
    const result = await invokeCapability(deps, 'run_apex_tests', { connection: 'data-only', class_names: ['InvoiceServiceTest'] });
    expect(result.isError).toBe(true);
    expect(textOf(result)).toContain('diagnostics_read');
  });
});
...
describe('get_flow_errors', () => {
  it('lists persisted interviews with the status field when available', async () => {
    const result = await invokeCapability(deps, 'get_flow_errors', { connection: 'data-org' });
    const parsed = JSON.parse(textOf(result)) as Record<string, unknown>;
