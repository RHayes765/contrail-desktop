...
export const dataCapabilities: Capability[] = [
  ...
  },
  {
    ...
    handler: (deps, rawArgs) =>
      guarded(async () => {
        ...
        const conn = requireConnection(deps, args.connection, 'explain_access');
        if (!/^[A-Za-z0-9_]+$/.test(args.object)) return fail('invalid object API name');
        let fieldName: string | null = null;
        if (args.field !== undefined) {
          const raw = args.field.includes('.') ? args.field.split('.').slice(-1)[0]! : args.field;
        ...
        const grantors: Array<{ name: string; assignees: number | null }> = [];
        ...
        return ok({
          ...
          ...(user.IsActive
            ? {}
            : { warning: 'This user is INACTIVE — they cannot log in regardless of permissions.' }),
          note:
  ...
  },
  {
    ...
    handler: (deps, rawArgs) =>
      guarded(async () => {
        ...
        const conn = requireConnection(deps, args.connection, 'get_report_data');
        const client = rest(deps, conn);
        const esc = (s: string) => s.replace(/\\/g, '\\\\').replace(/'/g, "\\'");
          ...
          if (candidates.length > 1) {
            return fail(
              `${candidates.length} reports share the DeveloperName "${leaf}" — pass the ` +
                `report id instead: ` +
                candidates
                  .map((r) => `${r.Name} [${r.FolderName ?? 'no folder'}] = ${r.Id}`)
                  .join(', '),
            );
          }
        ...
        try {
          const res = await client.request(
            `/services/data/${deps.config.salesforce.apiVersion}/analytics/reports/${reportId}` +
  ...
  },
  {
    ...
    handler: (deps, rawArgs) =>
      ...
      const conn = requireConnection(deps, args.connection, 'run_apex_tests');
      const client = rest(deps, conn);
      ...
      const res = await client.request(
        `/services/data/${deps.config.salesforce.apiVersion}/tooling/runTestsAsynchronous`,
  ...
  },
  {
    ...
    handler: (deps, rawArgs) =>
      guarded(async () => {
        const args = rawArgs as { connection: string; minutes?: number };
        const conn = requireConnection(deps, args.connection, 'set_trace_flag');
        const client = rest(deps, conn);
        const version = deps.config.salesforce.apiVersion;
...
function shapeReportRun(
  ...
  const { kept, dropped: budgetDropped } = fitToBudget(
    allRows.slice(0, rowCap),
    MAX_RESPONSE_CHARS,
  );
