 ...
 * Salesforce language servers (@contrail/apex-ls, see its PROVENANCE.md).
 * Ported from the plugin repo's tools/localdiag.ts — descriptions are
 * word-identical (prompt assets). grant: null — no org, no grant, the same
 * zero-cost class as list_connections; the catalog tripwire keeps grant-null
 * capabilities uncataloged, so these mint into every session.
 *
...
export const localDiagCapabilities: Capability[] = [
  {
    name: 'check_apex',
    ...
    handler: (deps, rawArgs) =>
      guarded(async () => {
        const args = rawArgs as { code: string; type?: 'class' | 'trigger' | 'anonymous' };
