...
import { readArtifactFromSnapshot } from '../metadata/read.js';
import type { Capability } from './types.js';
import { ok, fail, guarded } from './result.js';

/**
 * Semantic diff capabilities: cross-org comparison over local snapshots.
 ...
 * (Ported verbatim from the Phase 0 tool layer, tools/diff.ts.)
 */
...
function requireBoth(
  ...
  const a = deps.db.resolveConnection(refA);
  if (!a) throw new ConnectionNotFoundError(refA);
  const b = deps.db.resolveConnection(refB);
  if (!b) throw new ConnectionNotFoundError(refB);
  assertGrant(a, tool, deps.audit);
  assertGrant(b, tool, deps.audit);
  return [a, b];
}

export const diffCapabilities: Capability[] = [
  {
    name: 'diff_orgs',
    title: 'Diff two orgs',
    description:
    ...
    handler: (deps, rawArgs) =>
      guarded(() => {
        const args = rawArgs as { connection_a: string; connection_b: string; types?: string[] };
        const { db } = deps;
        if (args.types?.some((t) => !TYPE_RE.test(t))) return fail('invalid metadata type');
        const [a, b] = requireBoth(deps, args.connection_a, args.connection_b, 'diff_orgs');
