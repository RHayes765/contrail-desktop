...
import { ApprovalPageServer, type ApprovalPresenter } from './approval.js';
import {
  analyzeChanges,
...
export class DeployEngine {
  ...
  constructor(
    ...
    private readonly approvals: ApprovalPresenter = new ApprovalPageServer(),
    private readonly paths: { deploysDir: string } = { deploysDir: deploysDir() },
  ) {}
  ...
  private async doValidate(
    ...
    const zipPath = path.join(this.paths.deploysDir, `${request.id}.zip`);
  ...
  private async doExecute(
    ...
    // re-running the tests. This is both faster and STRICTER than replaying
    // the zip — nothing between validation and execution can substitute a
    // package. NoTestRun validations are not eligible (the org refuses), and
    // any org-side refusal (validation expired, coverage) falls back to the
    // classic full deploy of the stored zip.
    const quickEligible =
    ...
    const payload: Record<string, unknown> = {
      ...
      status: result.status,
      // Which execution path ran — the human asked for what they reviewed,
      // so the result says whether the org reused the validated package
      // (quick deploy) or the stored zip was re-deployed.
  ...
  async proposeDml(conn: ConnectionRecord, input: DmlInput): Promise<Record<string, unknown>> {
    ...
    const rowLabels = (
      preview.rows as Array<{ label: string; warnings: string[]; detail?: string; destructive?: boolean }>
    ).slice(0, 50);
    ...
    const results = isDmlPlan(input)
      ? [
          ...
          { label: 'Operations', value: summarizeOps(input.steps) },
          {
            label: 'Mode',
            value: allOrNone
   ...
   * surface carries the script verbatim plus the sharpest warning in the
   * product, and a compile error at execute spends the code exactly like a
   * failed DML.
   */
        ...
        changes: [
          {
            ...
            // bytes, so nothing may be elided (detail renders pre-wrap).
            detail: script,
          },
        ],
        destructive: [],
  ...
  async proposeBulkLoad(
    ...
    const stageDir = path.join(this.paths.deploysDir, `bulk-stage-${crypto.randomUUID()}`);
    ...
    try {
      ...
      const payloadDir = path.join(this.paths.deploysDir, request.id);
  ...
  private async doExecuteBulk(
    ...
    const resultsDir = path.join(this.paths.deploysDir, `${request.id}-results`);
...
function approvalForAgent(
  ...
  if (approval.mode === 'native') {
    ...
    return {
      ...
      mode: 'native',
      note:
        `The user sees this ${kindNoun(kind)} in the app's ` +
