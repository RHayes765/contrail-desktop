...
import { apexLsDir } from '../index.js';

/**
 * THE VENDOR TRIPWIRE (desktop mirror of the plugin's vendor.test.ts). The
 * repo's unanchored `dist/` gitignore would silently exclude
 * vendor/apex-ls/dist/** from CI's clean checkout without the negation —
 * shipping check_apex permanently "not_installed" with no error anywhere.
 * Pins live in vendor/PROVENANCE.md — update BOTH on a deliberate refresh.
 */
...
describe('vendored language-server payload', () => {
  it('resolves via apexLsDir() and matches the pinned bytes', () => {
    const dir = apexLsDir();
    for (const entry of EXPECTED) {
      const file = path.join(dir, entry.rel);
      expect(fs.existsSync(file), `${entry.rel} must exist (gitignore ate it?)`).toBe(true);
      const data = fs.readFileSync(file);
      if (entry.bytes >= 0) expect(data.length, `${entry.rel} byte size`).toBe(entry.bytes);
      if (entry.sha256) {
  ...
  it('the soql bin anchor pins CommonJS (ESM fallthrough crashes the bundle at startup)', () => {
    const anchor = JSON.parse(
      fs.readFileSync(path.join(apexLsDir(), 'bin', 'package.json'), 'utf8'),
    ) as { type?: string };
